...
async def test_accepts_a_real_java_access_token(client: AsyncClient):
    """A token from auth-service must open the API. The inbox is empty rather than
    unauthorised, which proves the token itself verified."""
    response = await client.get(INBOX, headers=auth(JAVA_ACCESS_TOKEN))
    assert response.status_code == 200
    assert response.json() == []


async def test_the_java_token_subject_is_the_inbox_owner(client: AsyncClient):
    """Identity comes from the sub claim, so a notification addressed to that userId is
    the one the real token sees."""
    from tests.conftest import internal_headers, notification_body

    created = await client.post(
        "/internal/notifications",
        json=notification_body(uuid.UUID(JAVA_USER_ID), title="For the Java subject"),
        headers=internal_headers(),
    )
    assert created.status_code == 201

    response = await client.get(INBOX, headers=auth(JAVA_ACCESS_TOKEN))
    assert [n["title"] for n in response.json()] == ["For the Java subject"]
...
async def test_java_refresh_token_cannot_authenticate(client: AsyncClient):
    ...
    response = await client.get(INBOX, headers=auth(JAVA_REFRESH_TOKEN))
    assert response.status_code == 401


async def test_expired_java_token_is_rejected(client: AsyncClient):
    response = await client.get(INBOX, headers=auth(JAVA_EXPIRED_TOKEN))
    assert response.status_code == 401


async def test_token_signed_with_another_secret_is_rejected(client: AsyncClient):
    token = make_token(uuid.uuid4(), "CUSTOMER", secret="a-different-secret-entirely-abcdef")
    response = await client.get(INBOX, headers=auth(token))
    assert response.status_code == 401
...
async def test_alg_none_is_rejected(client: AsyncClient):
    ...
    token = jwt.encode({"sub": str(uuid.uuid4()), "role": "CUSTOMER"}, key="", algorithm="none")
    response = await client.get(INBOX, headers=auth(token))
    assert response.status_code == 401


async def test_expired_token_is_rejected(client: AsyncClient):
    token = make_token(uuid.uuid4(), "CUSTOMER", expires_in=timedelta(minutes=-5))
    response = await client.get(INBOX, headers=auth(token))
    assert response.status_code == 401
...
async def test_token_with_non_uuid_subject_is_rejected(client: AsyncClient):
    token = jwt.encode(
        {"sub": "not-a-uuid", "role": "CUSTOMER", "exp": 4102444800},
        TEST_SECRET,
        algorithm="HS384",
    )
    response = await client.get(INBOX, headers=auth(token))
...
async def test_malformed_authorization_header_is_rejected(client: AsyncClient, header: str):
    response = await client.get(INBOX, headers={"Authorization": header})
    assert response.status_code == 401


@pytest.mark.parametrize("algorithm", ["HS256", "HS384", "HS512"])
async def test_whole_hmac_family_is_accepted(client: AsyncClient, algorithm: str):
    """The secret length decides the algorithm upstream, so all three must verify."""
    token = make_token(uuid.uuid4(), "CUSTOMER", algorithm=algorithm)
    response = await client.get(INBOX, headers=auth(token))
    assert response.status_code == 200
